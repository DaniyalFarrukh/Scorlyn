...
import { MonitorSmartphone, FileText, LogOut, ShieldCheck } from 'lucide-react';
...
export default function CourtLayout({
  ...
  const navItems = [
    { name: 'Live Controller', href: '/court', icon: MonitorSmartphone },
  ];
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col md:flex-row text-black font-sans">
      ...
      <aside className="w-full md:w-64 bg-white border-r border-gray-200 flex flex-col shadow-sm hidden md:flex sticky top-0 h-screen">
        <div className="h-16 flex items-center px-6 border-b border-gray-100">
          <Link href="/" className="text-xl font-black tracking-tighter uppercase flex items-center gap-2">
            <ShieldCheck size={20} className="text-[var(--color-brand-accent)]" />
            Scorlyn Court
          </Link>
        </div>
      ...
      <div className="md:hidden bg-white h-16 border-b border-gray-200 flex items-center justify-between px-4 sticky top-0 z-10 shadow-sm">
        <Link href="/court" className="text-lg font-black tracking-tighter uppercase flex items-center gap-2">
          <ShieldCheck size={18} />
          Scorlyn
        </Link>
